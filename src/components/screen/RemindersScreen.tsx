...
export default function RemindersScreen(_props: RemindersScreenProps) {
  ...
  const [mediums, setMediums] = React.useState<string[]>([]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Stack style={{ gap: 10, flexDirection: "row", alignItems: "center" }}>
          <Text style={{ fontSize: 20 }}>{time.toLocaleTimeString()}</Text>
          <Button
            radius={99999}
            type="clear"
            onPress={() => {
              DateTimePickerAndroid.open({
                ...
                display: "clock",
                value: time,
                onChange: (_, date) => setTime((time) => date || time),
              });
            }}
            size="sm"
